...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            now_gap = score0 - last_score
            if now_gap > running_high:
                print(now_gap, "point(s)! The most yet for Player 0")
                return announce_highest(0, score0, now_gap)
            else:
                return announce_highest(0, score0, running_high)
        else:
            now_gap = score1 - last_score
            if now_gap > running_high:
                print(now_gap, "point(s)! The most yet for Player 1")
                return announce_highest(1, score1, now_gap)
            else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_res, opt_dice_num = 0, 0
    num_dice = 1
    while num_dice <= 10:
        res = make_averaged(roll_dice, trials_count)(num_dice, dice)
        if res > max_res:
            max_res, opt_dice_num = res, num_dice
        num_dice = num_dice + 1
    return opt_dice_num
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    Take a strategy that have more rate to win as possible.
