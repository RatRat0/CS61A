...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            old_ant = place.ant
            new_ant = self
            old_ant_kind = isinstance(old_ant, ContainerAnt) and old_ant.can_contain(new_ant)
            new_ant_kind = isinstance(new_ant, ContainerAnt) and new_ant.can_contain(old_ant)
            assert old_ant_kind == True or new_ant_kind == True, 'Two ants in {0}'.format(place)
            if old_ant_kind:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt is an ant that does nothing each turn, but it has big armor, it is useful.
    """
    name = "Wall"
    food_cost = 4
    implemented = True
...
class ScubaThrower(ThrowerAnt):
    """an ant that can placed in the water
    """
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.is_only == False:
            QueenAnt.is_only = True
    ...
    def action(self, gamestate):
        ...
        if self.is_true_queen == True:
            ...
            at_place = self.place.exit
            while at_place:
                if at_place.ant != None and at_place.ant.is_doubled == False:
                    at_place.ant.is_doubled = True
                    at_place.ant.damage *= 2
                at_place = at_place.exit
        else:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.is_rec_direction == False:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while (cur_place is not None) and (cur_place is not beehive):
            if cur_place.ant is not None:
                ...
                if isinstance(cur_place.ant, ContainerAnt) and cur_place.ant.contained_ant is not None:
                    res[cur_place.ant.contained_ant] = distance
